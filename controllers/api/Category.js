...
export default class Controller {
    static create = async (request, response) => {
        try {
            const record = new Category(request.body)
    ...
    static listing = async (request, response) => {
        try {
            ...
            if (!id) {
                ...
                const records = await Category.findOne({ _id: id })
    ...
    static update = async (request, response) => {
        try {
            const result = await Category.findByIdAndUpdate(request.params.id, request.body)
            response.send(result)
        } catch (error) {
            response.send(error)
        }
